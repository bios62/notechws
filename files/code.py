...
debug_level = 0  #  Set to 1 to dump mem stats for each gc call
...
def post_to_rest(millicelsius):
    ...
    #
    payload = '{"temp":' + "{:.2f}".format(millicelsius / 1000) + "}"
    ...
    #
    # Twp types of error may occur, eiter a exception is thrown (network error)
    # or HTTP Eroor (HTTP-400 - HTTP-500)
    #
    try:
        headers = {"Content-Type": "application/json"}
        print("-" * 40)
        response = requests.post(REST_POST_URL, headers=headers, data=payload)
